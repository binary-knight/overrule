...
7. **Reconvene.** A closed meeting can be reopened with a new instruction from the meeting view. The same members carry on with the whole record: the transcript, the objections, the earlier candidate and ballots (shown to them as history), the workspace branch, and the attached documents. The new session gets its own cycle budget, its own draft and ballots, and its own entry in the record; earlier results stay under **Earlier sessions**. In a workspace meeting the next candidate builds on the branch: if you applied the previous one, the new candidate is only the delta; if you discarded it, the branch is recreated. Use this for work that does not fit one sitting, one stage at a time. **Resume** is different: it finishes an interrupted or failed session under the same instruction.

The implementer's time limit, set in the Workspace section, goes up to four hours; the candidate's check commands share it.
