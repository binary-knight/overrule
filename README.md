...
- **Where files live and when they go.** An upload waits under `data/attachments/staged/` (a day at most) until a meeting claims it, then sits under `data/attachments/<meeting id>/`. When the meeting finishes, the files and their extracted text are deleted before the result is announced. When a meeting fails, is stopped, or is interrupted, the uploaded files are deleted and only the extracted text is kept, so **Resume** still works; **Remove the text now** on the meeting deletes that too and ends resume for that meeting. Startup clears anything an unclean shutdown left behind. The meeting record keeps each document's name, size, SHA-256, and character count, never its text; what members quoted stays in the transcript like anything else they said.
