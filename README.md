...
`review` attaches a folder read-only and takes a report template; `ask` holds a plain meeting. Both wait for the verdict, print the report on standard output, and set the exit status: **0** approved, **2** objections remain because a member voted against, **3** checks failed, **4** verification incomplete, **5** unsettled because the meeting ran out of turns with nobody dissenting, **1** error. `resume <id> --cycles 3` buys an unsettled meeting more turns without paying for the openings again, and `reconvene <id> "<what next>"` opens a new session on the same record. With `--json` the output is one object holding the verdict (with the vote counts, whether anyone dissented, and why the meeting closed), the ballots, the objections with the condition that resolves each one, the candidate text, the members, and the meeting id, which is what an agent should read. `--no-wait` prints the id instead of waiting, and `watch` or `show` picks it up later. `--attach` sends a file, `--members` and `--drafter` pick the council, and `--level workspace-write` lets the drafter implement rather than only report.
