...
export function accessSection(run) {
  ...
  const line = p => {
    ...
    return `${p.name}: ${p.type === 'codex-cli' ? 'reads files and runs commands in a read-only sandbox; cannot write files' : 'reads and searches files; no shell, cannot run commands or write files'}${browse(p)}.`;
  };
