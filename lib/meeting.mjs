...
function cycleStats(run, active) {
  const turns = sessionTurns(run), size = Math.max(1, active.length);
...
export function currentCycle(run, active) { return 1 + Math.floor(sessionTurns(run).length / Math.max(1, active.length)); }
