...
import { rulesFor, activeMembers, plan, prompts, splitFields, normalizeFields, latestStance, record, metrics, stopReasonText, committed, currentSession } from './meeting.mjs';
...
export function validateRun(input, providers) {
  ...
  return { prompt, participants, drafterId, cycles, maxTokens, timeoutSeconds, maxRevisions, workspace: input.workspace || null };
}
...
export class Mesh {
  ...
  async reconvene(run, participants, { text, cycles }) {
    ...
    run.sessions = [...(run.sessions || []), { session, cycles: run.cycles, stopReason: run.stopReason, status: run.status, record: run.record || null, final: run.final || '', candidateVersion: run.candidateVersion, finishedAt: run.finishedAt || null, applied: ws?.applied || null, discarded: ws?.discarded || null }];
    if (ws && ws.level !== 'read-only') {
      // Applied: the next candidate is the delta from the branch tip, not the whole branch again. Discarded: the branch is gone; the draft step recreates it.
      if (ws.applied && ws.branch) { ws.base = (await git(ws.path, ['rev-parse', ws.branch])).trim(); ws.applied = null; }
      if (ws.discarded) { ws.branch = null; ws.base = null; ws.discarded = null; ws.impl = null; ws.review = null; }
    }
    Object.assign(run, { session: session + 1, cycles, status: 'running', phase: 'floor', stopReason: null, revisions: 0, candidateVersion: run.candidateVersion + 1, final: '', record: null, dropped: [], failures: {}, reconvenedAt: new Date().toISOString() });
    delete run.error; delete run.finishedAt;
    run.plannedCalls += plannedCalls(activeMembers(run).length, cycles);
