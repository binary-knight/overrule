...
import { newBudget, validateLimits, assertBudget, settleBudget, assertCoversImplementer, BudgetLimitError } from './budget.mjs';
...
export function validateRun(input, providers) {
  ...
  return { prompt, participants, drafterId, cycles, maxTokens, timeoutSeconds, maxRevisions, research, ...limits, workspace: input.workspace || null };
}
...
export class Mesh {
  ...
  resume(run, participants, limits = {}, changes = {}) {
    ...
    if (!['failed', 'interrupted', 'cancelled', 'limited'].includes(run.status)) throw new Error('Only a failed, interrupted, or stopped meeting can be resumed.');
  ...
  applySettings(run, { workspace, cycles, maxRevisions, research } = {}) {
    const changed = [];
    if (typeof research === 'boolean' && research !== (run.research === true)) {
      run.research = research;
      changed.push(`research mode ${research ? 'on: members may search the web' : 'off: no member may search the web'}`);
    }
    if (workspace) {
  ...
  start(run, options) {
    ...
    this.execute(run, options.participants, controller.signal).catch(async error => {
      ...
      run.status = cause instanceof BudgetLimitError ? 'limited' : controller.signal.aborted ? 'cancelled' : 'failed';
  ...
  async execute(run, participants, signal) {
    ...
    const context = (provider, action) => {
      const research = run.research === true ? { research: true } : {};
