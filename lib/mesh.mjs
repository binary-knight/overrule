...
export class Mesh {
  ...
  resume(run, participants, limits = {}, changes = {}) {
    ...
    // A meeting that ran out of turns is complete but not settled. Buying it more cycles continues the same session rather than
    // starting a new one; anything else about a finished meeting belongs to reconvene, which opens a session of its own.
    const moreCycles = Number.isInteger(changes.cycles) && changes.cycles > run.cycles && ['budget', 'stalled'].includes(run.stopReason);
    if (!['failed', 'interrupted', 'cancelled', 'limited', 'paused'].includes(run.status) && !(run.status === 'complete' && moreCycles)) {
      throw new Error(run.status === 'complete'
        ? `This meeting finished${run.stopReason ? ` (${run.stopReason})` : ''}. Give it more cycles than the ${run.cycles} it had to carry on with the same question, or reconvene it with a new instruction.`
        : 'Only a paused, failed, interrupted, or stopped meeting can be resumed.');
    }
  ...
  applySettings(run, { workspace, cycles, maxRevisions, research, deepResearch, timeoutSeconds } = {}) {
    ...
    if (workspace) {
      ...
      if (String(merged.exclude || []) !== String(old.exclude || [])) changed.push((merged.exclude || []).length ? `out of scope: ${merged.exclude.join(', ')}` : 'nothing out of scope');
    ...
    if (Number.isInteger(cycles) && cycles !== run.cycles) {
      ...
      // Reopening the floor means the candidate and its ballots belong to a debate that has moved on: the next draft is a new version.
      if (more && ['budget', 'stalled'].includes(run.stopReason)) {
        run.stopReason = null; run.phase = 'floor'; run.revisions = 0;
        if (committed(run).some(e => e.phase === 'draft' && e.candidateVersion === run.candidateVersion)) { run.candidateVersion++; run.final = ''; run.record = null; }
        changed.push('the floor reopens');
      }
    }
  ...
  async execute(run, participants, signal) {
    ...
    const context = (provider, action) => {
      ...
      if (action.type === 'ratify' && ws.review && existsSync(ws.review)) return { ...research, cwd: ws.review, level: ws.level === 'full-access' ? 'full-access' : 'read-only', claudeShell: ws.claudeSandbox !== false };
      // Floor turns: read-only under a confined level; at full access every tool-bearing member is loose, as the owner chose.
      return { ...research, cwd: ws.path, level: ws.level === 'full-access' ? 'full-access' : 'read-only', claudeShell: ws.claudeSandbox !== false };
    };
