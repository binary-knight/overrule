...
export class Mesh {
  constructor(store, providerCall = callProvider, tools = {}) {
    this.store = store; this.providerCall = providerCall; this.runChecks = tools.runChecks || runChecks;
  ...
  create(options, demo = false) {
    ...
    const run = {
      id: randomUUID(), createdAt: new Date().toISOString(), status: 'running', phase: 'opening', kind: 'meeting', prompt: options.prompt,
  ...
  start(run, options) {
    ...
    this.publish(run);
    this.execute(run, options.participants, controller.signal).catch(error => {
      run.status = controller.signal.aborted ? 'cancelled' : 'failed';
      run.error = error.message; this.publish(run);
  ...
  async execute(run, participants, signal) {
    ...
    const invoke = async (provider, action, extra = {}) => {
      ...
      try {
        const prompt = prompts(run, action.type === 'revise' ? { ...action, type: 'draft' } : action, extra);
