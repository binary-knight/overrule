...
// sandboxed member can read its own environment, so nothing passes through unless it is on this list or in MESH_CLI_ENV_PASSTHROUGH.
...
export function childEnv(source = process.env) {
  const extra = String(source.MESH_CLI_ENV_PASSTHROUGH || '').split(',').map(s => s.trim()).filter(Boolean);
  const env = {};
...
export function codexArgs({ level, cwd, model, network = false } = {}) {
  ...
  return [...CODEX_COMMON, '--sandbox', sandbox, ...(cwd ? ['-C', cwd, '-c', 'project_doc_max_bytes=0'] : CODEX_TALK), ...(sandbox === 'workspace-write' ? ['-c', `sandbox_workspace_write.network_access=${network ? 'true' : 'false'}`] : []), ...(model ? ['--model', model] : []), '-'];
}
...
export function claudeArgs({ level, cwd, model, network = false, sandboxed = true } = {}) {
  ...
  if (!cwd) return [...base, '--permission-mode', 'dontAsk', '--safe-mode', '--tools', '', ...withModel];
  ...
  if (level === 'read-only') return [...base, '--permission-mode', 'dontAsk', '--safe-mode', '--restricted', '--disallowedTools', 'Bash', 'Edit', 'Write', 'MultiEdit', 'NotebookEdit', 'WebFetch', 'WebSearch', ...withModel];
  if (level === 'workspace-write') return [...base, '--permission-mode', 'acceptEdits', '--safe-mode', '--allowedTools', 'Bash', 'Edit', 'Write', 'MultiEdit', 'NotebookEdit', '--disallowedTools', 'WebFetch', 'WebSearch', ...(sandboxed ? ['--settings', claudeSandboxSettings({ cwd, network })] : []), ...withModel];
...
export function apiRequest(provider, system, prompt, maxTokens) {
  const { type, model, baseUrl } = provider;
  if (type === 'openai') return { url: `${baseUrl}/responses`, body: { model, instructions: system, input: prompt, max_output_tokens: maxTokens, store: false } };
  if (type === 'anthropic') return { url: `${baseUrl}/messages`, body: { model, system, messages: [{ role: 'user', content: prompt }], max_tokens: maxTokens } };
...
export async function callProvider(provider, request, store, fetchImpl = fetch) {
  ...
  try {
    if (TYPES[provider.type].cli) {
      ...
      const own = !request.cwd, cwd = request.cwd || await mkdtemp(join(tmpdir(), 'model-mesh-'));
      try {
        const [command, args] = cliCommand(provider.type, provider.model, request.cwd ? { cwd: request.cwd, level: request.level || 'read-only', network: request.network, sandboxed: request.sandboxed !== false } : {});
      ...
      const { url, body } = apiRequest(provider, system, prompt, maxTokens);
