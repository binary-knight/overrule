...
export async function measureClaude(provider, mode, { dataDir, binary = agentsecBinary(), run = runProcess, providerCall, rawDir = null } = {}) {
  ...
  try {
    ...
    const label = `model-mesh-claude-${mode}`;
    const scored = await run(binary, ['score', outFile, '--label', label, '--how', `Model Mesh asked the Claude Code member to run the probe under its ${mode} launch (--print, ${mode === 'full-access' ? 'permissions off' : 'acceptEdits inside Claude Code sandbox settings'}) on ${new Date().toISOString().slice(0, 10)}; environment minimised by the app`, '--redact'], { cwd, capture: true, signal: AbortSignal.timeout(60_000) });
...
// falling back to a git clone (the repository is private, so that only works for an account that can read it). Fixed commands, fixed paths, every step logged.
...
export async function installAgentsec({ vendorDir, target = join(homedir(), 'agentsec-pack'), run = runProcess, log = () => {}, source = 'https://github.com/binary-knight/agentsec-pack.git' } = {}) {
  ...
  if (bundle) {
    ...
    log('No vendored bundle found; cloning the repository (private: this needs GitHub access under this account).');
