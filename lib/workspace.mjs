...
// An allowed root is a directory the owner named in the app; every workspace must sit inside one. A root may contain the app's own
// folder (a projects directory usually does); that folder itself is still refused as a workspace.
export function validateRoot(input, rules) { return realDirectory(input, { ...rules, browsing: true }); }

export function validateWorkspacePath(input, { roots = [], appRoot, dataDir, browsing = false }) {
  if (!roots.length) throw new Error('No allowed directories yet. Add one in the Workspace section first.');
  const real = realDirectory(input, { appRoot, dataDir, browsing });
  const under = roots.map(r => { try { return realpathSync(resolve(r)); } catch { return null; } }).filter(Boolean).find(r => real === r || real.startsWith(r + sep));
  if (!under) throw new Error(`The workspace must be inside an allowed directory: ${roots.join(', ')}.`);
  return real;
}

// Subdirectories of an allowed directory, for picking a workspace from a browser on another machine. Hidden and dependency folders are skipped.
export function browse(input, rules) {
  const path = input ? validateWorkspacePath(input, { ...rules, roots: rules.roots, browsing: true }) : null;
  const list = dir => readdirSync(dir, { withFileTypes: true }).filter(e => e.isDirectory() && !e.name.startsWith('.') && !SKIP_DIRS.has(e.name)).map(e => ({ name: e.name, path: join(dir, e.name), git: existsSync(join(dir, e.name, '.git')) })).sort((a, b) => a.name.localeCompare(b.name)).slice(0, 200);
  if (!path) return { path: null, entries: rules.roots.map(r => ({ name: r, path: r, git: existsSync(join(r, '.git')), root: true })) };
  return { path, git: existsSync(join(path, '.git')), entries: list(path) };
}

// Browse the host to pick a directory to allow. Starts at the home directory; hidden and dependency folders are skipped;
// each entry says whether it could be allowed, so system directories and the app's own folders show but cannot be chosen.
export function browseHost(input, rules) {
  ...
  if (!statSync(real).isDirectory()) throw new Error('That path is not a directory.');
  const selectable = p => { try { validateRoot(p, rules); return true; } catch { return false; } };
  let entries = [];
...
export async function applyCandidate(path, branch) {
  ...
  await git(path, ['-c', 'user.name=Model Mesh', '-c', 'user.email=mesh@localhost', 'merge', '--no-ff', '--no-verify', '-m', `Apply council candidate from ${branch}`, branch]);
...
export async function runChecks(commands, cwd, { network = false, timeoutSeconds = 600, run = runProcess, signal } = {}) {
  ...
  for (const command of commands) {
    ...
    try {
      const { code, stdout, stderr } = await run('codex', ['sandbox', '-c', 'sandbox_mode="workspace-write"', '-c', `sandbox_workspace_write.network_access=${network ? 'true' : 'false'}`, '--', 'bash', '-lc', command], { cwd, signal: signal || AbortSignal.timeout(timeoutSeconds * 1000), capture: true });
      results.push({ command, code, output: (stdout + (stderr ? `\n[stderr]\n${stderr}` : '')).slice(-6000), seconds: Math.round((Date.now() - started) / 1000) });
    } catch (error) { results.push({ command, code: null, output: error.message, seconds: Math.round((Date.now() - started) / 1000) }); }
  }
  return results;
}
