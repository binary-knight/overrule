...
// Word, PDF, spreadsheet and slide files are binary: a member with a shell can unzip one, a member without one cannot read it at
// all. Naming them lets the owner attach them, which is the only way every member sees the same thing.
const BINARY_DOCUMENT = /\.(pdf|docx?|xlsx?|pptx?|odt|ods|odp|rtf)$/i;
export function binaryDocuments(path, limit = 25) {
  const found = [];
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      else if (BINARY_DOCUMENT.test(entry.name)) { try { found.push({ path: full, name: entry.name, size: statSync(full).size }); } catch {} }
    }
  };
  walk(path, 0);
  return found;
...
export async function inspect(path) {
  const info = { path, name: basename(path), git: false, documents: binaryDocuments(path), branch: null, head: null, dirty: false, dirtyFiles: [], secrets: scanSecrets(path) };
  try {
...
export async function createBranch(path, runId) {
  const branch = `overrule/${runId.slice(0, 8)}`;
...
export async function commitCandidate(worktree, message, author = 'Overrule <overrule@localhost>') {
  ...
  await git(worktree, ['-c', `user.name=Overrule`, '-c', 'user.email=overrule@localhost', 'commit', '--quiet', '--no-verify', '--author', author, '-m', message]);
...
export async function applyCandidate(path, branch, hash) {
  ...
  await git(path, ['-c', 'user.name=Overrule', '-c', 'user.email=overrule@localhost', 'merge', '--no-ff', '--no-verify', '-m', `Apply council candidate ${hash} from ${branch}`, hash]);
...
export function agentsecBinary(env = process.env) {
  const candidates = [env.OVERRULE_AGENTSEC, env.MESH_AGENTSEC, join(homedir(), 'agentsec-pack', '.venv', 'bin', 'agentsec'), join(homedir(), '.local', 'bin', 'agentsec')].filter(Boolean);
...
export async function measureLevel(level, cwd, options = {}) {
  return measureTemplate(levelTemplate(level), `overrule-${level}`, cwd, { ...options, level });
}
